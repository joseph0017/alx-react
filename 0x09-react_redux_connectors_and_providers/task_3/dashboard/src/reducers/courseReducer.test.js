...
describe('courseReducer', () => {
  it('Test that the default state returns an empty array', () => {
    expect(defaultState).toEqual(Map([]));
  });
  ...
  it('Test that SELECT_COURSE returns the data with the right item updated', () => {
    ...
    expect(courseReducer(selectedCourseData, selectCourse(2)).toJS()).toEqual(getData.toJS());
  });

  it('Test that UNSELECT_COURSE returns the data with the right item updated', () => {
    const UnSelectedCourseData = Map([
      {
        id: 1,
        name: 'ES6',
        isSelected: false,
    ...
    expect(courseReducer(UnSelectedCourseData, unSelectCourse(2)).toJS()).toEqual(getData.toJS());
  });
});
